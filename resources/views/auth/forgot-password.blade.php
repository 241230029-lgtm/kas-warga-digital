...
<div class="min-h-[calc(100vh-160px)] flex items-center justify-center py-16 px-4">
    <div class="max-w-md w-full bg-white rounded-[40px] shadow-2xl border border-gray-100 p-10">
        <h1 class="text-4xl font-extrabold text-gray-900 mb-4">Reset Kata Sandi</h1>
        <p class="text-gray-500 mb-8">Masukkan email Anda untuk menerima tautan reset kata sandi.</p>
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
            ...
            <button type="submit" class="w-full bg-emerald-500 text-white rounded-3xl py-4 font-semibold shadow-lg shadow-emerald-200 hover:bg-emerald-600 transition">Kirim Tautan Reset</button>

            <p class="text-sm text-gray-500 text-center mt-4">Ingat kata sandi? <a href="{{ route('login') }}" class="font-semibold text-emerald-600 hover:text-emerald-700">Masuk</a>.</p>
        </form>
    </div>
</div>
@endsection
